...
# ---------------------------------------------------------------------------
# Step 2: OMP post hook (symlink preferred, copy fallback)
# ---------------------------------------------------------------------------


def _omp_wrapper():
    """Hook content for symlink-less installs: pins NOTIFIER_NG_INGEST to
    this checkout and imports the real adapter from it. The adapter resolves
    that variable lazily (at delivery time, never at module load), so the
    assignment is honored even though ESM evaluates the imported adapter
    before this wrapper's body runs. Re-run ``install.py --apply`` after
    moving the checkout."""
    return (
        "/** Generated by install.py (OMP hook; symlink unsupported here).\n"
        " * Pins the notifier-ng checkout: the adapter resolves\n"
        " * NOTIFIER_NG_INGEST lazily, before each ingest, so this override\n"
        " * is honored despite ESM evaluating the imported adapter first.\n"
        " * Re-run install.py --apply after moving the checkout.\n"
        " */\n"
...
def step_omp_hook(plan, hooks_post_dir):
    ...
    if target.is_symlink():
        try:
            if target.resolve() == source.resolve():
                plan.noop(f"OMP hook symlink already installed: {target}")
                return
        except OSError:
            pass
        plan.refuse(
        ...
        return
    elif same_content or wrapper_installed:
        plan.noop(f"OMP hook already installed with identical content: {target}")
        return
    elif target.exists():
        if target.is_dir():
        ...
        plan._tag("create", f"symlink {target}", f"-> {source}")
    ...
    try:
        ...
        # A bare copy cannot resolve the core: the adapter's sibling-relative
        # fallback would look next to the hook, not the checkout. Write a
        # minimal wrapper that pins NOTIFIER_NG_INGEST to this checkout and
        # imports the real adapter from it, so the installed hook keeps
        # resolving this checkout without any machine-specific path.
        _atomic_write(target, _omp_wrapper(), mode=0o644)
        plan._tag("create", f"file {target}", "symlink failed; wrote wrapper resolving this checkout")
